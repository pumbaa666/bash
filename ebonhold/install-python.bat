...
:install_dependencies
    ...
    if errorlevel 1 (
        echo Error: Failed to upgrade pip in Python virtual environment.
        goto :exit_error
    )

    echo Installing dependencies (requests) in virtual environment...
    venv\Scripts\python -m pip install requests --disable-pip-version-check >nul 2>&1
    if errorlevel 1 (
        echo Error: Failed to install dependencies in Python virtual environment.
        goto :exit_error
    )

    venv\Scripts\python -c "import requests" >nul 2>&1
    if errorlevel 1 (
        echo Error: Failed to import dependencies in Python virtual environment.
        goto :exit_error
    )
...
:install_python
    ...
    for %%R in (64 32) do (
        for /f "tokens=2*" %%A in (
            'reg query "HKLM\Software\Python\PythonCore\%PYTHON_VERSION_MAJOR%\InstallPath" /ve /reg:%%R 2^>nul'
        ) do if not defined PYTHON_EXE set "PYTHON_EXE=%%Bpython.exe"
    )
